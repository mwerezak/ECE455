...
void software_delay(unsigned long delay_ms)
{
	...
	for(i = 0; i < numiter; i++); //note: second for loop is in runSoftwareTimer()
...
{
	//LPC_TIM0->TCR = 0x02Í¾ // reset timer
	//LPC_TIM0->TCR = 0x01Í¾ // enable timer
	//LPC_TIM0->MR0 = 2048Í¾ // match value (can be anything)
	//LPC_TIM0->MCR |= 0x03Í¾ // on match, generate interrupt and reset
